#!/usr/bin/env bash
# =============================================================================
#  build-kong-3.9.2-1.31.1-rpm-arm64.sh
#  Build Kong OSS 3.9.2 arm64 .rpm using SuriyaRuk/kong-build-tools
#  OpenResty: 1.31.1.1   |   Base OS: Amazon Linux 2023
#
#  Output: output/kong-3.9.2-openresty1.31.1.1.arm64.rpm
# =============================================================================
set -euo pipefail
...
BUILD_TOOLS_DIR="/tmp/kong-build-tools-392-rpm-arm64"
KONG_SOURCE_DIR="/tmp/kong-source-392-rpm-arm64"
...
PACKAGE_TYPE="rpm"
DOCKER_REPOSITORY="kong-build-local-rpm-arm64"
...
# ─── 0. Set up QEMU for arm64 emulation (if needed) ───────────────────────────
HOST_ARCH="$(uname -m)"
case "${HOST_ARCH}" in
  arm64|aarch64)
    log "Host is ${HOST_ARCH}; arm64 build is native — skipping QEMU registration." ;;
  *)
    log "Host is ${HOST_ARCH}; registering QEMU for arm64 emulation ..."
...
# ─── 7b. Build local kong/fpm:0.5.1 image for arm64 ──────────────────────────
...
# ─── 7d. Ensure arm64 alpine base ────────────────────────────────────────────
log "Ensuring arm64 alpine base image ..."
docker pull --platform=linux/arm64 alpine:latest >/dev/null

# ─── 8. Run make package-kong ─────────────────────────────────────────────────
log "Starting Kong arm64 RPM build ..."
